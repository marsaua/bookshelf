document.addEventListener("turbo:load", () => {
  ...
  input.addEventListener("input", async () => {
    ...
    const res = await fetch(
      `/books/search?query=${encodeURIComponent(input.value)}`,
    );
  ...
  window.selectBook = (book) => {
    ...
    document.querySelector('[name="book[description]"]').value =
      book.description;
    document.querySelector('[name="book[cover_url]"]').value = book.image;
    document.querySelector('[name="book[category]"]').value =
      book.category || "";
    document.querySelector('[name="book[page_count]"]').value =
      book.page_count || "";
    document.querySelector('[name="book[language]"]').value =
      book.language || "";
    document.querySelector('[name="book[publisher]"]').value =
      book.publisher || "";
