class BookRequestsController < ApplicationController
    ...
    def accept
      ...
      @book_request.book.update(lent_to_user_id: @book_request.requester_id)
    ...
    def set_book_request
        if action_name.in?(%w[accept decline])
          @book_request = BookRequest.joins(:book).where(books: { user_id: current_user.id }).find(params[:id])
        else
          @book_request = current_user.book_requests.find(params[:id])
        end
    end
